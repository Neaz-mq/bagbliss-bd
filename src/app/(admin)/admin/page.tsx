...
const STATUS_CONFIG: Record<StatusKey, { bg: string; text: string; dot: string; border: string; label: string }> = {
  ...
  processing: { bg: 'rgba(var(--color-accent-rgb),0.08)', text: 'var(--color-accent-dark)', dot: 'var(--color-accent)', border: 'rgba(var(--color-accent-rgb),0.25)', label: 'Processing' },
...
const STAT_CARDS = [
  { label: 'Total Revenue',   key: 'totalRevenue',   icon: TrendingUp,  accent: 'var(--color-accent)',  accentBg: 'rgba(var(--color-accent-rgb),0.1)',  gradient: 'linear-gradient(135deg, var(--color-accent) 0%, var(--color-accent-dark) 100%)', isCurrency: true,  change: '+12.5%', up: true, desc: 'vs last month' },
  { label: 'Total Orders',    key: 'totalOrders',    icon: ShoppingBag, accent: 'var(--color-primary)', accentBg: 'rgba(var(--color-primary-rgb),0.08)', gradient: 'linear-gradient(135deg, var(--color-primary) 0%, var(--color-primary-light) 100%)', isCurrency: false, change: '+8.2%',  up: true, desc: 'vs last month' },
  { label: 'Active Products', key: 'totalProducts',  icon: Package,     accent: 'var(--color-gold-dark)', accentBg: 'rgba(var(--color-gold-rgb),0.12)',  gradient: 'linear-gradient(135deg, var(--color-gold) 0%, var(--color-gold-dark) 100%)', isCurrency: false, change: '+3',     up: true, desc: 'new this month' },
  { label: 'Customers',       key: 'totalCustomers', icon: Users,       accent: 'var(--color-accent-dark)', accentBg: 'rgba(var(--color-accent-rgb),0.1)', gradient: 'linear-gradient(135deg, var(--color-accent-light) 0%, var(--color-accent) 100%)', isCurrency: false, change: '+18.7%', up: true, desc: 'vs last month' },
] as const
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px', padding: '4px 0 8px' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: 'rgba(var(--color-accent-rgb),0.08)', border: '1px solid rgba(var(--color-accent-rgb),0.2)', borderRadius: '999px', padding: '3px 12px 3px 8px' }}>
              <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'var(--color-accent)', boxShadow: '0 0 6px rgba(var(--color-accent-rgb),0.8)', display: 'inline-block' }} />
              <span style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--color-accent-dark)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Live Dashboard</span>
            </div>
          </div>
          <h1 style={{ fontSize: '1.85rem', fontWeight: 800, color: 'var(--color-primary)', margin: 0, letterSpacing: '-0.03em', lineHeight: 1.15 }}>
            {greeting}, {firstName}
          </h1>
        ...
        <Link href="/admin/orders" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '10px 20px', borderRadius: '12px', fontSize: '0.85rem', fontWeight: 700, color: 'white', textDecoration: 'none', background: 'linear-gradient(135deg, var(--color-accent), var(--color-accent-dark))', boxShadow: '0 4px 16px rgba(var(--color-accent-rgb),0.4)' }}>
        ...
        {STAT_CARDS.map(({ label, key, icon: Icon, accent, accentBg, gradient, isCurrency, change, up, desc }) => {
          ...
          return (
            <div key={label} style={{ background: '#ffffff', borderRadius: '20px', padding: '24px', border: '1px solid #f1f5f9', boxShadow: '0 1px 2px rgba(0,0,0,0.04)', position: 'relative', overflow: 'hidden' }}>
              ...
              <p style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--color-primary)', margin: 0, lineHeight: 1, letterSpacing: '-0.04em' }}>{value}</p>
        ...
        <div style={{ background: '#ffffff', borderRadius: '20px', border: '1px solid #f1f5f9', boxShadow: '0 1px 2px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '1px solid #f8fafc' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'rgba(var(--color-accent-rgb),0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <BarChart3 size={16} color="var(--color-accent)" />
              </div>
              <div>
                <p style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--color-primary)', margin: 0 }}>Recent Orders</p>
                <p style={{ fontSize: '0.72rem', color: '#94a3b8', margin: '2px 0 0' }}>Last 5 transactions</p>
              </div>
            </div>
            <Link href="/admin/orders" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '0.78rem', fontWeight: 700, color: 'var(--color-accent-dark)', textDecoration: 'none', background: 'rgba(var(--color-accent-rgb),0.07)', border: '1px solid rgba(var(--color-accent-rgb),0.15)', padding: '6px 12px', borderRadius: '8px' }}>
            ...
            stats.recentOrders.map((order, i) => {
              ...
              return (
                <div key={order._id} style={{ display: 'grid', gridTemplateColumns: '1fr 140px 120px 100px', padding: '14px 24px', borderBottom: i < stats.recentOrders.length - 1 ? '1px solid #f8fafc' : 'none', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '10px', flexShrink: 0, background: 'linear-gradient(135deg, rgba(var(--color-accent-rgb),0.1), rgba(var(--color-accent-dark-rgb),0.06))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <p style={{ fontSize: '0.9rem', fontWeight: 800, color: 'var(--color-primary)', margin: 0, textAlign: 'right' }}>৳{order.total.toLocaleString('en-BD')}</p>
                </div>
              )
            })
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 24px', textAlign: 'center' }}>
              <div style={{ width: '56px', height: '56px', borderRadius: '16px', background: 'rgba(var(--color-accent-rgb),0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '14px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ background: '#ffffff', borderRadius: '20px', border: '1px solid #f1f5f9', boxShadow: '0 1px 2px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 20px 14px', borderBottom: '1px solid #f8fafc' }}>
              <p style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--color-primary)', margin: 0 }}>Order Status</p>
          ...
          <div style={{ background: '#ffffff', borderRadius: '20px', border: '1px solid #f1f5f9', boxShadow: '0 1px 2px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 20px 12px', borderBottom: '1px solid #f8fafc' }}>
              <p style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--color-primary)', margin: 0 }}>Quick Actions</p>
            </div>
            <QuickActions />
          </div>
