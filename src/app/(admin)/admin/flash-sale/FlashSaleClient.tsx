...
function EditPriceModal({
  ...
  const [price, setPrice] = useState(
    String(product.flashSalePrice || Math.round(product.price * 0.8))
  )
  ...
  const saving_ = async () => {
    ...
    if (Number(price) >= product.price) {
      toast.error('Flash price must be lower than regular price')
      return
    }
    setSaving(true)
    try {
          ...
          {Number(price) >= product.price && Number(price) > 0 && (
            ...
            >
              ...
              <span style={{ fontSize: '0.8rem', color: '#b91c1c' }}>
                Flash price must be lower than ৳{product.price.toLocaleString()}
              </span>
            </div>
          )}
        </div>
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              opacity:
                saving || Number(price) >= product.price || !price ? 0.6 : 1,
      ...
      await Promise.all(
        [...selected].map((id) =>
          fetch(`/api/admin/flash-sale/${id}`, {
            ...
            body: JSON.stringify({
              isFlashSale: true,
              flashSalePrice: Number(prices[id]),
            }),
          })
        )
      )
      toast.success(
        `${selected.size} product${selected.size > 1 ? 's' : ''} added to flash sale!`
      )
      onAdded()
      onClose()
    } catch {
            ...
            <div style={{ padding: '40px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '0.875rem', color: '#94a3b8', margin: 0 }}>
                {search
                  ...
                  : 'All active products are already in the flash sale'}
              </p>
            </div>
          ) : (
            available.map((p) => {
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: '12px',
                    marginBottom: '4px',
                    background: sel ? 'rgba(233,30,140,0.04)' : 'transparent',
                    border: `1.5px solid ${sel ? 'rgba(233,30,140,0.2)' : 'transparent'}`,
                    cursor: 'pointer',
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      height: '20px',
                      borderRadius: '6px',
                      border: `2px solid ${sel ? '#e91e8c' : '#e2e8f0'}`,
                      background: sel ? '#e91e8c' : '#fff',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      flexShrink: 0,
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      Regular: ৳{p.price.toLocaleString()} ·{' '}
        ...
        >
          ...
          >
            {saving ? (
              ...
              <>
                <Zap size={14} /> Add {selected.size > 0 ? selected.size : ''}{' '}
                to Flash Sale
              </>
            )}
          </button>
...
export default function FlashSaleClient() {
  ...
  const handlePriceSaved = (id: string, newPrice: number) => {
    setProducts((prev) =>
      prev.map((p) => (p._id === id ? { ...p, flashSalePrice: newPrice } : p))
    )
  }
  ...
  const filtered = products
    ...
    .sort((a, b) => {
      if (sortBy === 'discount') {
        return discount(a.originalPrice || a.price, a.flashSalePrice) <
          discount(b.originalPrice || b.price, b.flashSalePrice)
          ? 1
          : -1
      }
  ...
  const avgDiscount = products.length
    ? Math.round(
        products.reduce(
          (a, p) => a + discount(p.originalPrice || p.price, p.flashSalePrice),
          0
        ) / products.length
      )
    : 0
  const totalSavings = products.reduce(
    (a, p) => a + ((p.originalPrice || p.price) - p.flashSalePrice),
    0
  )
        ...
        {[
          {
            ...
            icon: Zap,
            gradient: 'linear-gradient(135deg, #e91e8c, #f43f5e)',
          ...
          },
          {
            ...
            value: products.reduce((a, p) => a + p.totalStock, 0),
            icon: ShoppingBag,
        ...
        >
          ...
          >
            {search
              ...
              : 'Add products to start your flash sale and attract customers'}
          </p>
          {!search && (
            <button
              onClick={() => setAddModal(true)}
