...
import {
  Zap, Package, Search, X, Plus, Trash2,
  RefreshCw, ChevronDown, Save, AlertTriangle,
  TrendingDown, Tag, ShoppingBag, Loader2,
} from 'lucide-react'
...
function EditPriceModal({
  ...
  product: Product
  onClose: () => void
  onSaved: (id: string, flashPrice: number) => void
}) {
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: '#fff', borderRadius: '20px', width: '100%', maxWidth: '440px', boxShadow: '0 24px 64px rgba(0,0,0,0.22)', overflow: 'hidden' }}
      >
        {/* Header */}
        ...
        <div style={{ padding: '22px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #f1f5f9' }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <p style={{ fontSize: '0.75rem', color: '#94a3b8', margin: '3px 0 0' }}>
                Regular price: <strong style={{ color: '#475569' }}>৳{product.price.toLocaleString()}</strong>
                {product.originalPrice > product.price && (
                  <span style={{ textDecoration: 'line-through', marginLeft: '6px', color: '#94a3b8' }}>৳{product.originalPrice.toLocaleString()}</span>
                )}
              </p>
            </div>
          </div>
          ...
          {Number(price) > 0 && Number(price) < product.price && (
            <div style={{ padding: '12px 14px', borderRadius: '10px', background: 'rgba(233,30,140,0.05)', border: '1px solid rgba(233,30,140,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: '0.78rem', color: '#94a3b8', textDecoration: 'line-through' }}>৳{product.price.toLocaleString()}</span>
                <span style={{ fontSize: '0.9rem', fontWeight: 800, color: '#e91e8c' }}>৳{Number(price).toLocaleString()}</span>
                <span style={{ padding: '2px 8px', borderRadius: '6px', background: '#e91e8c', color: 'white', fontSize: '0.72rem', fontWeight: 800 }}>
                  -{disc}%
                </span>
              </div>
            </div>
          ...
          {Number(price) >= product.price && Number(price) > 0 && (
            <div style={{ padding: '10px 14px', borderRadius: '10px', background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '0.8rem', color: '#b91c1c' }}>Flash price must be lower than ৳{product.price.toLocaleString()}</span>
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', padding: '16px 22px', borderTop: '1px solid #f1f5f9', background: '#fafbfc' }}>
          ...
          <button
            onClick={saving_} disabled={saving || Number(price) >= product.price || !price}
...
function AddProductsModal({
  ...
  return (
    ...
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{ background: '#fff', borderRadius: '20px', width: '100%', maxWidth: '600px', maxHeight: '88vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.22)', overflow: 'hidden' }}
      >
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: '1px solid #f1f5f9', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '0.72rem', color: '#94a3b8', margin: '2px 0 0' }}>{selected.size} selected</p>
            </div>
          </div>
          <button onClick={onClose} style={{ width: '32px', height: '32px', borderRadius: '9px', border: '1.5px solid #e2e8f0', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#64748b' }}>
            <X size={14} />
          ...
          {loading ? (
            ...
            available.map(p => {
              ...
              return (
                <div
                  key={p._id}
                  style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 10px', borderRadius: '12px', marginBottom: '4px', background: sel ? 'rgba(233,30,140,0.04)' : 'transparent', border: `1.5px solid ${sel ? 'rgba(233,30,140,0.2)' : 'transparent'}`, cursor: 'pointer', transition: 'all 0.12s' }}
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: '0.72rem', color: '#94a3b8', margin: '2px 0 0' }}>
                      Regular: ৳{p.price.toLocaleString()} · {CAT_LABELS[p.category] ?? p.category}
                    </p>
                  </div>
...
export default function FlashSaleClient() {
  ...
  const filtered = products
    ...
    .sort((a, b) => {
      if (sortBy === 'discount') {
        return discount(a.originalPrice || a.price, a.flashSalePrice) < discount(b.originalPrice || b.price, b.flashSalePrice) ? 1 : -1
      }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={fetchFlashSale} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '9px 16px', borderRadius: '10px', border: '1.5px solid #e2e8f0', background: '#f8fafc', color: '#475569', cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '14px' }}>
        {[
          ...
          { label: 'Total Savings',value: `৳${totalSavings.toLocaleString()}`, icon: Tag, gradient: 'linear-gradient(135deg, #10b981, #059669)', bg: 'rgba(16,185,129,0.08)' },
          ...
          <div key={label} style={{ background: '#fff', borderRadius: '16px', padding: '18px', border: '1px solid #f1f5f9', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '0.72rem', color: '#64748b', margin: '4px 0 0', fontWeight: 500 }}>{label}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: '1 1 200px', background: '#fff', border: '1.5px solid #e8edf5', borderRadius: '12px', padding: '0 14px', height: '42px' }}>
          ...
          <input type="text" placeholder="Search flash sale products…" value={search} onChange={e => setSearch(e.target.value)}
            style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontSize: '0.85rem', color: '#334155' }} />
      ...
      {loading ? (
        ...
        <div style={{ padding: '80px 20px', textAlign: 'center', background: '#fff', borderRadius: '20px', border: '1px solid #f1f5f9' }}>
          ...
          {!search && (
            <button onClick={() => setAddModal(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '10px 24px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #e91e8c, #f43f5e)', color: 'white', fontSize: '0.875rem', fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 14px rgba(233,30,140,0.35)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '14px' }}>
          {filtered.map(p => {
            ...
            return (
              <div key={p._id} style={{ background: '#fff', borderRadius: '16px', border: '1px solid #f1f5f9', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', overflow: 'hidden', transition: 'box-shadow 0.2s' }}
              ...
              >
                ...
                <div style={{ padding: '14px' }}>
                  <p style={{ fontSize: '0.78rem', color: '#94a3b8', margin: '0 0 4px', fontWeight: 600 }}>
                    {CAT_LABELS[p.category] ?? p.category}
                  </p>
                  <p style={{ fontSize: '0.95rem', fontWeight: 700, color: '#1e293b', margin: '0 0 10px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {p.name}
                  </p>

                  {/* Prices */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                    <span style={{ fontSize: '1.2rem', fontWeight: 800, color: '#e91e8c' }}>
                    ...
                    <span style={{ fontSize: '0.7rem', fontWeight: 700, padding: '2px 7px', borderRadius: '6px', background: 'rgba(34,197,94,0.08)', color: '#15803d', border: '1px solid rgba(34,197,94,0.2)', marginLeft: 'auto' }}>
                      Save ৳{(p.price - p.flashSalePrice).toLocaleString()}
                    </span>
                  </div>
