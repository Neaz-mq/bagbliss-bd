...
const CAT_STYLE: Record<string, { bg: string; text: string }> = {
  'mini-crossbody': { bg: 'rgba(233,30,140,0.08)', text: '#be185d' },
  'chain-strap':    { bg: 'rgba(99,102,241,0.08)', text: '#4338ca' },
  'leather':        { bg: 'rgba(180,120,60,0.1)',  text: '#7c4b1e' },
  'canvas':         { bg: 'rgba(16,185,129,0.08)', text: '#065f46' },
  'party':          { bg: 'rgba(245,158,11,0.08)', text: '#92400e' },
}
...
function FInput({
  ...
  return (
    <input
      ...
      style={{
        width: '100%', padding: '9px 12px', border: '1.5px solid #e8edf5',
    ...
    <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
      <button
        ...
        style={{
          ...
          background: checked ? '#e91e8c' : '#e2e8f0',
...
function ProductModal({
  ...
  const updColor  = (i: number, f: string, v: string) =>
    set('colors', form.colors.map((c, idx) => idx === i ? { ...c, [f]: v } : c))
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        images:           form.images,
        colors:           form.colors.map(c => ({ name: c.name, hex: c.hex, stock: Number(c.stock) || 0 })),
  ...
  return (
    <div
      onClick={onClose}
      style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(15,23,42,0.65)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{ background: '#fff', borderRadius: '20px', width: '100%', maxWidth: '700px', maxHeight: '92vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.22)', overflow: 'hidden' }}
      >
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 24px', borderBottom: '1px solid #f1f5f9', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <p style={{ fontSize: '1rem', fontWeight: 800, color: '#0f172a', margin: 0 }}>
                {product ? 'Edit Product' : 'Add New Product'}
              </p>
              <p style={{ fontSize: '0.72rem', color: '#94a3b8', margin: '2px 0 0' }}>
                {product ? product.name : 'Add a new bag to your store'}
              </p>
            </div>
          </div>
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <section>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <div>
                <p style={{ fontSize: '0.72rem', fontWeight: 700, color: '#64748b', margin: '0 0 6px' }}>Sale Price (৳) *</p>
                <FInput placeholder="e.g. 850" value={form.price} onChange={v => set('price', v)} type="number" />
              </div>
              <div>
                <p style={{ fontSize: '0.72rem', fontWeight: 700, color: '#64748b', margin: '0 0 6px' }}>Original Price (৳)</p>
                <FInput placeholder="e.g. 1200" value={form.originalPrice} onChange={v => set('originalPrice', v)} type="number" />
              </div>
            </div>
            {disc > 0 && (
              <div style={{ marginTop: '8px', padding: '7px 12px', borderRadius: '8px', background: 'rgba(34,197,94,0.07)', border: '1px solid rgba(34,197,94,0.2)', fontSize: '0.78rem', color: '#15803d', fontWeight: 600 }}>
                💰 {disc}% discount will be shown to customers
              </div>
            )}
          </section>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: '6px', flex: '1 1 200px' }}>
                <input
                  ...
                  onBlur={e  => (e.target.style.borderColor = '#e8edf5')}
                />
                <button onClick={addUrl} type="button" style={{ padding: '8px 14px', borderRadius: '10px', border: 'none', background: '#e91e8c', color: 'white', fontSize: '0.82rem', fontWeight: 700, cursor: 'pointer' }}>
                  Add
                </button>
              </div>
            </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(88px, 1fr))', gap: '8px' }}>
                  {form.images.map((url, i) => (
                    <div key={i} style={{ position: 'relative', aspectRatio: '1', borderRadius: '10px', overflow: 'hidden', border: `2px solid ${i === 0 ? '#e91e8c' : '#f1f5f9'}` }}>
                      ...
                      {i === 0 && (
                        <span style={{ position: 'absolute', top: '4px', left: '4px', background: '#e91e8c', color: 'white', fontSize: '0.52rem', fontWeight: 800, padding: '2px 5px', borderRadius: '4px' }}>
                          MAIN
                        </span>
                      )}
                      <button
                        onClick={() => removeImg(i)}
                        style={{ position: 'absolute', top: '4px', right: '4px', width: '20px', height: '20px', borderRadius: '50%', background: 'rgba(15,23,42,0.75)', border: 'none', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                      >
                        <X size={10} />
                      </button>
                    </div>
          ...
          <section>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
              ...
              <button
                ...
                style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '5px 12px', borderRadius: '8px', border: '1.5px solid rgba(233,30,140,0.25)', background: 'rgba(233,30,140,0.06)', fontSize: '0.78rem', fontWeight: 700, color: '#e91e8c', cursor: 'pointer' }}
              >
                ...
                {form.colors.map((c, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <input type="color" value={c.hex} onChange={e => updColor(i, 'hex', e.target.value)}
                      style={{ width: '38px', height: '38px', border: 'none', borderRadius: '8px', cursor: 'pointer', padding: '2px', flexShrink: 0 }} />
                    <input
                      placeholder="Color name (e.g. Rose Pink)"
                      value={c.name} onChange={e => updColor(i, 'name', e.target.value)}
                    ...
                    />
                    <input
                      type="number" placeholder="Stock" value={c.stock} min="0"
                      ...
                      onBlur={e  => (e.target.style.borderColor = '#e8edf5')}
                    />
                    <button onClick={() => rmColor(i)} style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1.5px solid #fee2e2', background: '#fff1f2', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#ef4444', flexShrink: 0 }}>
                      <X size={14} />
                    </button>
                  </div>
                ))}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '10px', padding: '16px 24px', borderTop: '1px solid #f1f5f9', flexShrink: 0, background: '#fafbfc' }}>
          <button onClick={onClose} style={{ padding: '9px 20px', borderRadius: '10px', border: '1.5px solid #e2e8f0', background: '#fff', fontSize: '0.875rem', fontWeight: 600, color: '#475569', cursor: 'pointer' }}>
            Cancel
          </button>
          <button
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <p style={{ fontSize: '0.875rem', color: '#94a3b8', margin: '4px 0 0' }}>
            {total} total · {activeCount} active{lowStock > 0 ? ` · ⚠️ ${lowStock} low stock` : ''}
          </p>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={fetchProducts} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '9px 16px', borderRadius: '10px', border: '1.5px solid #e2e8f0', background: '#f8fafc', color: '#475569', cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600 }}>
            <RefreshCw size={14} /> Refresh
          </button>
          <button onClick={openAdd} style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', padding: '9px 18px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #e91e8c, #f43f5e)', color: 'white', fontSize: '0.875rem', fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 14px rgba(233,30,140,0.35)' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
        ...
        {[
          { val: catFilter,  set: setCat,  opts: [{ v: '', l: 'All Categories' }, ...CATEGORIES.map(c => ({ v: c.value, l: `${c.emoji} ${c.label}` }))] },
          { val: statFilter, set: setStat, opts: [{ v: '', l: 'All Status' }, { v: 'active', l: 'Active' }, { v: 'inactive', l: 'Inactive' }] },
        ].map((sel, i) => (
      ...
      <div style={{ background: '#fff', borderRadius: '20px', border: '1px solid #f1f5f9', boxShadow: '0 1px 2px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
        {/* Table head */}
        <div style={{ display: 'grid', gridTemplateColumns: '56px 1fr 130px 110px 80px 80px 88px', padding: '11px 20px', fontSize: '0.65rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em', background: '#fafbfc', borderBottom: '1px solid #f1f5f9' }}>
          <span>Img</span><span>Product</span><span>Category</span>
        ...
        {!loading && products.length === 0 && (
          <div style={{ padding: '80px 20px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '0.85rem', color: '#94a3b8', margin: '6px 0 16px' }}>
        ...
        {!loading && products.map((p, i) => {
          ...
          const cs      = CAT_STYLE[p.category] ?? { bg: '#f1f5f9', text: '#475569' }
          ...
          return (
            <div
              key={p._id}
              style={{ display: 'grid', gridTemplateColumns: '56px 1fr 130px 110px 80px 80px 88px', padding: '12px 20px', borderBottom: i < products.length - 1 ? '1px solid #f8fafc' : 'none', alignItems: 'center' }}
              onMouseEnter={e => (e.currentTarget.style.background = '#fafbfc')}
              onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
            >
              {/* Image */}
              <div style={{ width: '44px', height: '44px', borderRadius: '10px', overflow: 'hidden', background: 'rgba(233,30,140,0.05)', border: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {p.images[0]
              ...
              <div style={{ minWidth: 0, paddingRight: '12px' }}>
                ...
                <p style={{ fontSize: '0.68rem', color: '#94a3b8', margin: '2px 0 0', fontFamily: 'monospace' }}>/{p.slug}</p>
                {p.isFeatured && <span style={{ fontSize: '0.6rem', fontWeight: 700, padding: '1px 6px', borderRadius: '4px', background: 'rgba(245,158,11,0.1)', color: '#b45309' }}>⭐ Featured</span>}
              </div>

              {/* Category */}
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '0.72rem', fontWeight: 700, padding: '4px 9px', borderRadius: '7px', background: cs.bg, color: cs.text, width: 'fit-content' }}>
                {cat?.emoji} {cat?.label ?? p.category}
              </span>

              {/* Price */}
              <div>
                <p style={{ fontSize: '0.9rem', fontWeight: 800, color: '#e91e8c', margin: 0 }}>৳{p.price.toLocaleString()}</p>
              ...
              {/* Stock */}
              <div>
                <p style={{ fontSize: '0.875rem', fontWeight: 700, margin: 0, color: isOut ? '#ef4444' : isLow ? '#f59e0b' : '#15803d' }}>{p.totalStock}</p>
                <p style={{ fontSize: '0.65rem', color: '#94a3b8', margin: '1px 0 0' }}>{isOut ? 'Out of stock' : isLow ? 'Low stock' : 'In stock'}</p>
              </div>
              ...
              <div>
                <button
                  ...
                  style={{ width: '42px', height: '23px', borderRadius: '11px', background: p.isActive ? '#e91e8c' : '#e2e8f0', border: 'none', cursor: 'pointer', position: 'relative', transition: 'background 0.2s' }}
                >
                  <span style={{ position: 'absolute', top: '2.5px', left: p.isActive ? '21px' : '2.5px', width: '18px', height: '18px', borderRadius: '50%', background: 'white', transition: 'left 0.2s', boxShadow: '0 1px 4px rgba(0,0,0,0.15)' }} />
                </button>
                <p style={{ fontSize: '0.62rem', color: '#94a3b8', margin: '2px 0 0' }}>{p.isActive ? 'Visible' : 'Hidden'}</p>
              </div>

              {/* Actions */}
              <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                {[
                  { icon: Edit2, action: () => openEdit(p), hover: { bg: 'rgba(99,102,241,0.07)', border: 'rgba(99,102,241,0.3)', color: '#6366f1' } },
                  { icon: Trash2, action: () => setDelId(p._id), hover: { bg: '#fff1f2', border: 'rgba(239,68,68,0.3)', color: '#ef4444' } },
                ].map(({ icon: Icon, action, hover }, bi) => (
                  <button
                    key={bi} onClick={action}
                    style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1.5px solid #e2e8f0', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#64748b' }}
                    onMouseEnter={e => { e.currentTarget.style.background = hover.bg; e.currentTarget.style.borderColor = hover.border; e.currentTarget.style.color = hover.color }}
      ...
      {pages > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          <p style={{ fontSize: '0.82rem', color: '#64748b', margin: 0 }}>
            Showing <strong style={{ color: '#1e293b' }}>{(page-1)*limit+1}–{Math.min(page*limit,total)}</strong> of <strong style={{ color: '#1e293b' }}>{total}</strong>
          </p>
          <div style={{ display: 'flex', gap: '6px' }}>
            <button onClick={() => setPage(p => Math.max(1,p-1))} disabled={page===1} style={{ width: '36px', height: '36px', borderRadius: '10px', border: '1.5px solid #e2e8f0', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: page===1?'not-allowed':'pointer', opacity: page===1?0.4:1, color: '#475569' }}>
              <ChevronLeft size={15} />
            </button>
            {Array.from({ length: Math.min(5, pages) }, (_, i) => {
              ...
              return (
                <button key={n} onClick={() => setPage(n)} style={{ width: '36px', height: '36px', borderRadius: '10px', border: `1.5px solid ${page===n?'rgba(233,30,140,0.3)':'#e2e8f0'}`, background: page===n?'rgba(233,30,140,0.08)':'#fff', color: page===n?'#e91e8c':'#475569', fontWeight: page===n?800:500, fontSize: '0.82rem', cursor: 'pointer' }}>
                  {n}
                </button>
              )
            })}
            <button onClick={() => setPage(p => Math.min(pages,p+1))} disabled={page===pages} style={{ width: '36px', height: '36px', borderRadius: '10px', border: '1.5px solid #e2e8f0', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: page===pages?'not-allowed':'pointer', opacity: page===pages?0.4:1, color: '#475569' }}>
      ...
      {delId && (
        <div onClick={() => setDelId(null)} style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(15,23,42,0.65)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '16px', padding: '28px', maxWidth: '360px', width: '100%', boxShadow: '0 24px 48px rgba(0,0,0,0.2)', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '0.875rem', color: '#64748b', margin: '0 0 24px', lineHeight: 1.6 }}>This will permanently delete the product and cannot be undone.</p>
