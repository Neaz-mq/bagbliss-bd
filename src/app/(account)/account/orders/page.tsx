...
export default function OrdersPage() {
          ...
          <div className="orders-skeleton" style={{ paddingTop: '2rem' }}>
            {[1, 2, 3].map((i) => (
              <div key={i} className="skeleton"
                style={{ height: '140px', borderRadius: '1.25rem', marginBottom: '1rem' }} />
            ))}
          </div>
        </div>
      </div>
    ...
    return (
      <div className="orders-empty-page">
        <div className="orders-empty-inner">
          <div className="orders-empty-icon"><Package size={40} strokeWidth={1.5} /></div>
          <h1 className="orders-empty-title">Sign in to view orders</h1>
          <p className="orders-empty-desc">Track your purchases from your account.</p>
  ...
  if (orders.length === 0) {
    return (
      <div className="orders-empty-page">
        <div className="orders-empty-inner">
          <div className="orders-empty-icon"><ShoppingBag size={40} strokeWidth={1.5} /></div>
                  ...
                  {order.items.map((item, i) => (
                    <div key={i} className="order-item-row">
                      ...
                      <div className="order-item-info">
                        ...
                        <span className="order-item-qty">Qty: {item.quantity} · {item.color}</span>
                      </div>
